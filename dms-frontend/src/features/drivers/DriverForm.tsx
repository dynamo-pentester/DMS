...
interface DriverFormProps {
  driver?: Driver | null;
  /** `photo` is only populated on create - the inline photo picker below has no effect on edit */
...
export function DriverForm({ driver, onSubmit, onCancel, loading = false }: DriverFormProps) {
  ...
  // Driver Photo is only collected on create - the picker below only renders
  // when !isEdit, and Replace on an existing driver still goes through the
  // View Details page.
  ...
  const handleFormSubmit = (values: DriverFormValues) => {
    ...
    onSubmit(payload, isEdit ? undefined : photo);
  };
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5">
      ...
      {/* Driver Photo (Create Only) - uploaded together with the driver record on submit */}
      {!isEdit && (
        <div>
