...
const REPORT_CARDS: ReportCard[] = [
  ...
  { id: "PlantMovement",   title: "Plant Movement Log",        description: "Vehicle entry/exit records per plant for audit and gate security review.", icon: "🏭", color: "text-teal-700",  gradientFrom: "from-teal-400",  gradientTo: "to-emerald-600"},
...
export function ReportsCenter() {
  ...
  const handleDownload = async (report: ReportCard) => {
    try {
      ...
      const url = `${baseUrl}/api/reports/${report.id.toLowerCase()}?${qs}`;
      ...
      toast.error("Report download failed. The report endpoint may not be available yet.");
    } finally {
  ...
  const hasFilters = dateFrom || dateTo || driverId || plantId;
    ...
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950 p-5 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Driver ID</label>
