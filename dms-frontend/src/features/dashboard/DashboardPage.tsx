...
export function DashboardPage() {
  ...
  // Sorting handler for Manager team table
  ...
  // Manager Pending Approvals — fetched from real /approvals/pending API
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      try {
        ...
        const managersCount = users.filter((u: any) =>
          Array.isArray(u.roles) && u.roles.includes("HOD")
        ).length;

        // ---- Compute pending approvals count for Admin stat card (compliance items needing action) ----
        // (Admin does not see driver approval requests — those are Manager-only)
        ...
        if (isManager && user) {
          ...
          if (myTransporter) {
            ...
            // Manager activity feed (only their drivers' incidents)
          }
        }
      } catch (err) {
    ...
    // ---- Manager: fetch pending driver approvals from real API ----
    if (isManager) {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (activeModal === "createDriver") {
        ...
        toast.success("HOD account registered successfully!");
  ...
  const handleApprovalAction = async (approvalId: number, action: "approve" | "reject") => {
    try {
      if (action === "approve") {
        await api.put(`/approvals/${approvalId}/approve`, { comments: null });
        toast.success("Driver approved successfully");
      } else {
        await api.put(`/approvals/${approvalId}/reject`, { comments: null });
        toast.success("Driver request rejected");
      }
      // Refresh pending approvals
      const approvalsRes = await api.get("/approvals/pending").catch(() => ({ data: [] }));
      setPendingApprovals(approvalsRes.data || []);
    } catch (err: any) {
      toast.error(err.response?.data?.error || "Failed to process approval action.");
    }
  };
      ...
      {isAdmin && (
        <>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
              <div className="flex items-center justify-between">
                <p className="text-xs font-medium uppercase tracking-wider text-slate-400">HODs</p>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
            {/* Left Column: Recent Drivers List */}
            <div className="lg:col-span-2 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  <h4 className="text-base font-semibold text-slate-900">Recent Drivers</h4>
                  <p className="text-xs text-slate-500">Latest enrolled drivers across all transporters</p>
                </div>
                <button
              ...
              <div className="overflow-x-auto border border-slate-100 rounded-xl">
                <table className="w-full text-left text-xs text-slate-650">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {rawDrivers.slice(0, 8).length > 0 ? (
                      rawDrivers.slice(0, 8).map((d: any) => (
                        <tr key={d.driverId} className="hover:bg-slate-50/50 transition-colors">
                          <td className="p-3 font-mono font-semibold text-slate-600">{d.driverCode}</td>
                          <td className="p-3 font-semibold text-slate-800">{d.fullName}</td>
                          <td className="p-3 text-slate-550">{d.currentTransporterName || <span className="text-slate-300">—</span>}</td>
                          <td className="p-3">
                            <CompactBadge text={d.currentStatusName} status={d.currentStatusName} />
                          </td>
                          <td className="p-3">
                            <CompactBadge
                              text={d.approvalStatus === "PendingApproval" ? "Pending" : (d.approvalStatus || "—")}
                              status={d.approvalStatus === "PendingApproval" ? "pending" : (d.approvalStatus?.toLowerCase() || "")}
                            />
                          </td>
                        </tr>
                      ))
                      ...
                      <tr>
                        <td colSpan={5} className="p-8 text-center text-slate-400">No drivers registered yet.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
      ...
      {isManager && (
        <>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-6">
            ...
            {/* Pending Driver Approvals card — real API count */}
            <div className="rounded-2xl border border-amber-100 bg-amber-50/40 p-6 shadow-sm cursor-pointer transition-all hover:shadow-md hover:border-amber-200" onClick={() => navigate("/approvals")}>
              <div className="flex items-center justify-between">
                <p className="text-xs font-medium uppercase tracking-wider text-amber-600">Pending Approvals</p>
                <div className="rounded-lg bg-amber-100 p-2 text-amber-600"><Briefcase size={16} /></div>
              </div>
              <h3 className="mt-4 text-3xl font-bold text-amber-700">{pendingApprovals.length}</h3>
              <p className="mt-1 text-xs text-amber-500">Driver requests awaiting action</p>
            </div>
          ...
          {/* Pending Driver Approvals — Primary block for HOD */}
          <div className="rounded-2xl border border-amber-100 bg-white p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-base font-semibold text-slate-900">Pending Driver Approvals</h4>
                <p className="text-xs text-slate-500">Employee-submitted driver enrollment requests requiring your action</p>
              </div>
              <div className="flex items-center gap-3">
                {pendingApprovals.length > 0 && (
                  <span className="inline-flex h-6 min-w-[1.5rem] items-center justify-center rounded-full bg-amber-100 px-2 text-xs font-bold text-amber-700">
                    {pendingApprovals.length}
                  </span>
                )}
                <button onClick={() => navigate("/approvals")} className="text-xs font-semibold text-amber-600 hover:text-amber-700">View All</button>
              </div>
            </div>
                  ...
                  {pendingApprovals.length > 0 ? (
                    pendingApprovals.map((req: any) => (
                      <tr key={req.id} className="hover:bg-amber-50/30 transition-colors">
                        <td className="p-3 font-mono font-semibold text-slate-600">{req.driverCode}</td>
                        <td className="p-3 font-semibold text-slate-800">{req.driverName}</td>
                        <td className="p-3 text-slate-550">{req.requestedByName || `User #${req.requestedByUserId}`}</td>
                        <td className="p-3 text-slate-450">
                          {req.requestedDate ? new Date(req.requestedDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "—"}
                        </td>
                        <td className="p-3 text-right space-x-2 whitespace-nowrap">
                          ...
                          <button
                            ...
                            className="rounded-lg bg-rose-600 px-2.5 py-1 text-[10px] font-semibold text-white hover:bg-rose-700"
                          >
                            Reject
                          </button>
                        </td>
                    ...
                    <tr>
                      <td colSpan={5} className="p-8 text-center text-slate-400">
                        ✓ No pending driver approvals. All employee submissions have been reviewed.
                      </td>
                    </tr>
                  )}
                </tbody>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {myTeam.slice(0, 5).map((d) => (
                      <tr key={d.id} className="hover:bg-slate-50/50">
                        ...
                        <td className="p-3 text-slate-550">—</td>
                        <td className="p-3">
          ...
          <div className="w-full max-w-md rounded-2xl border border-slate-100 bg-white p-6 shadow-xl animate-fade-in space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-900">
                ...
                {activeModal === "assignManager" && "Assign HOD"}
                ...
                {activeModal === "viewRequest" && "Driver Details"}
              </h3>
              <button
                onClick={() => {
                  setActiveModal(null);
            ...
            {/* View Request — Driver Availability Detail */}
            {activeModal === "viewRequest" && selectedRequest && (
              <div className="space-y-4 text-xs">
                <div>
                  <p className="text-slate-400 font-semibold uppercase tracking-wider">Driver</p>
                  <p className="text-sm font-semibold text-slate-800 mt-1">{selectedRequest.driver}</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-slate-400 font-semibold uppercase tracking-wider">Vehicle</p>
                    <p className="text-slate-800 mt-1">{selectedRequest.assignedVehicle}</p>
                  </div>
                  <div>
                    <p className="text-slate-400 font-semibold uppercase tracking-wider">Availability</p>
                    <div className="mt-1"><CompactBadge text={selectedRequest.availability} status={selectedRequest.availability} /></div>
                  </div>
                </div>
                <div>
                  <p className="text-slate-400 font-semibold uppercase tracking-wider">Today's Task</p>
                  <p className="text-slate-700 bg-slate-50 rounded-lg p-3 mt-1.5 leading-relaxed">{selectedRequest.todayTask}</p>
                </div>
