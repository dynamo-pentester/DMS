...
export function LicensesList() {
  ...
  useEffect(() => {
    async function loadLookups() {
      try {
        const [vt, end] = await Promise.all([lookupService.getVehicleTypes(), lookupService.getEndorsements()]);
        setVehicleTypes(vt);
        setEndorsements(end);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        await licenseService.create({ ...formData, driverId: Number(formData.driverId), vehicleTypeId: Number(formData.vehicleTypeId), endorsementIds: formData.endorsementIds.map(Number) } as CreateLicenseRequest);
