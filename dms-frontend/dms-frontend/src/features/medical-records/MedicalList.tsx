...
export function MedicalList() {
  ...
  const openCreateForm = () => {
    setEditingItem(null);
    setFormData({ driverId: "", examDate: "", fitnessStatusId: "", bp: "", visionTestPass: true, alcoholTestPass: true, chronicIllness: false, chronicIllnessRemarks: "", validTill: "" });
    setFormOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        await medicalService.create({ ...formData, driverId: Number(formData.driverId), fitnessStatusId: Number(formData.fitnessStatusId) });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {searchParams.status && (
          <button onClick={() => setSearchParams(p => ({ ...p, status: undefined, page: 1 }))} className="inline-flex items-center gap-1 text-xs font-semibold text-slate-500">
