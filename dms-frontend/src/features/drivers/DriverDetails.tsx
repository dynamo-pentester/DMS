...
export function DriverDetails({ driver, onRefresh }: DriverDetailsProps) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900">
      ...
      {/* Driver photo - upload / preview / view / download / replace */}
      <div className="mb-6 max-w-md">
        <FileUpload
          label="Driver Photo"
          variant="photo"
          hasFile={driver.hasPhoto}
          fileUrl={`/drivers/${driver.driverId}/photo`}
          accept="image/jpeg,image/png"
          hint="JPG or PNG"
          onUpload={async (file) => {
            await driverService.uploadPhoto(driver.driverId, file);
            onRefresh();
          }}
        />
      </div>
