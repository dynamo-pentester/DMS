...
export function IncidentsList() {
  ...
  const [caData, setCaData] = useState<any>({ description: "", actionDate: "", closedDate: "" });

  const [expandedRow, setExpandedRow] = useState<number | null>(null);
  const [formData, setFormData] = useState<any>({});

  useEffect(() => {
    lookupService.getIncidentTypes().then(setIncidentTypes).catch(console.error);
  }, []);
  ...
  const openCreateForm = () => {
    setEditingItem(null);
    setFormData({ driverId: "", incidentTypeId: "", incidentDate: "", location: "", description: "", severity: "", faultAtDriver: false, fatalitiesCount: 0, injuriesCount: 0 });
    setFormOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        await incidentService.create({ ...formData, driverId: Number(formData.driverId), incidentTypeId: Number(formData.incidentTypeId), fatalitiesCount: Number(formData.fatalitiesCount), injuriesCount: Number(formData.injuriesCount) });
  ...
  const handleAddCA = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await incidentService.addCorrectiveAction(caFormOpen.incidentId, { ...caData, incidentId: caFormOpen.incidentId });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-end gap-3">
        <button onClick={() => setFilterDrawerOpen(true)} className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
          ...
          {searchParams.severity && <span className="ml-1 h-2 w-2 rounded-full bg-blue-600" />}
        </button>
      </div>
                      ...
                      {row.correctiveActions.map((ca: CorrectiveAction) => (
                        <div key={ca.correctiveActionId} className="flex items-start gap-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-3">
                          <div className="flex-1">
                            <p className="text-xs text-slate-700 dark:text-slate-300">{ca.description}</p>
                            <p className="text-[10px] text-slate-400 mt-0.5">{format(new Date(ca.actionDate), "dd MMM yyyy")} {ca.closedDate ? `→ Closed: ${format(new Date(ca.closedDate), "dd MMM yyyy")}` : "• Open"}</p>
                          </div>
                          <StatusBadge status={ca.closedDate ? "Closed" : "Open"} />
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <AppModal isOpen={!!caFormOpen} onClose={() => setCaFormOpen(null)} title="Add Corrective Action">
        <form onSubmit={handleAddCA} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500">Description *</label>
            <textarea rows={3} required value={caData.description} onChange={e => setCaData({ ...caData, description: e.target.value })}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500">Closed Date</label>
              <input type="date" value={caData.closedDate} onChange={e => setCaData({ ...caData, closedDate: e.target.value })}
                className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm dark:border-slate-800 dark:bg-slate-950 dark:text-white" />
            </div>
          </div>
          <div className="flex justify-end gap-3 border-t border-slate-100 pt-4 dark:border-slate-800">
