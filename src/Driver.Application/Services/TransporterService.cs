...
public class TransporterService : ITransporterService
{
    ...
    public TransporterService(IApplicationDbContext db, INotificationService notifications)
    {
        _db = db;
    ...
    public async Task<TransporterDto> CreateAsync(CreateTransporterRequest request, int createdByUserId)
    {
        ...
        var created = await GetByIdAsync(transporter.TransporterId)
            ?? throw new InvalidOperationException("Transporter was created but could not be reloaded.");

        // Notify Manager that a new transporter was registered
        await _notifications.RaiseAsync(
            entityTypeName: "System",
            entityId: transporter.TransporterId,
            title: "New transporter registered",
            message: $"Transporter '{transporter.Name}' has been added to the system.",
