...
public class LookupsController : ControllerBase
{
    ...
    // Picking a LicenseNo here resolves DriverId/DriverCode/FullName so the form can
    // auto-fill the rest of the driver's details client-side.
    [HttpGet("licenses")]
    public async Task<IActionResult> Licenses()
        => Ok(await _db.Licenses
            .Include(l => l.Driver)
            .OrderByDescending(l => l.ValidTill)
            .Select(l => new
            {
                l.LicenseId,
                l.LicenseNo,
                l.DriverId,
                DriverCode = l.Driver!.DriverCode,
                DriverFullName = l.Driver!.FullName,
                l.ValidTill
            })
            .ToListAsync());
